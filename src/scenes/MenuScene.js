...
export class MenuScene extends Phaser.Scene {
    ...
    difficulties.forEach((diff, i) => {
      ...
      btn.on('pointerdown', () => {
        ...
        diffBtns.forEach((b, j) => {
          const bgGraphic = b.getAt(0);
          const txt = b.getAt(1);
          const sel = difficulties[j] === diff;
          bgGraphic.clear();
          if (sel) {
            bgGraphic.fillStyle(diffColors[difficulties[j]], 0.2);
            bgGraphic.fillRoundedRect(-35, -14, 70, 28, 14);
            bgGraphic.lineStyle(2, diffColors[difficulties[j]], 1);
            bgGraphic.strokeRoundedRect(-35, -14, 70, 28, 14);
            txt.setColor('#ffffff');
          } else {
            bgGraphic.fillStyle(0xffffff, 0.06);
            bgGraphic.fillRoundedRect(-35, -14, 70, 28, 14);
            txt.setColor('#9999aa');
          }
        });
  ...
  createDiffButton(x, y, label, color, selected) {
